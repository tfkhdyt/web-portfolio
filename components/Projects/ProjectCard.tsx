...
const ProjectCard = ({
  ...
  return (
    ...
    >
      <div
        className={`relative aspect-video h-44 w-full rounded-2xl transition duration-500 ${
          isHovered ? 'shadow-xl' : 'shadow-lg'
        }`}
      >
        <div
          className={`absolute inset-0 bottom-0 z-10 grid aspect-video place-items-center rounded-2xl bg-black/75 px-4 py-3 text-center text-white transition duration-500 ${
          ...
          <div>
            ...
            <div className='space-x-1'>
              {tags.map((tag) => (
                <div
                  data-theme='light'
                  className='badge badge-sm badge-info py-2 font-bold transition duration-500 hover:brightness-75'
                  key={tag}
                >
                  {tag}
                </div>
